...
def process_ribo_reads(h5, ribo_metadata, min_len=20, max_len=40):
    ...
    read_lens = np.arange(min_len, max_len + 1)
    ...
    for experiment in experiments:
        ...
        df = pl.read_csv(
            f"ribo/{experiment}/out/{experiment}_aligned_tran.sam",
            has_header=False,
            comment_char="@",
            columns=usecols,
            sep="\t",
        ...
        df = df.filter(
            (pl.col("read_len") <= max_len) & (pl.col("read_len") >= min_len)
        )
        mask_f = np.isin(tr_ids_f, df["tr_ID"].unique().to_numpy().astype("S"))

        print("Constructing empty datasets...")
        sparse_array = [
            sparse.csr_matrix((read_len_l, w)) for w in tqdm(tr_lens_f[~mask_f])
        ]
        ...
        for tr_id, group in tqdm(df.groupby("tr_ID"), total=len(df["tr_ID"].unique())):
            ...
            for row in group.rows():
                tr_reads[row[3] - min_len, row[1] - 1] += 1
